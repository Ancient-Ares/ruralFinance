            ...
            <scroller lock-x scrollbar-y use-pullup @on-pullup-loading="loadMore1" v-model="status1" ref="scroller1" class="scrollWarp" style="height: 100%;">
              <div class="box">
                <div class="listItem" v-for="(item, index) in ApprovalList">
                  ...
                  <div class="itemBot">
                    <p>{{item.isDoBusiness===0?'审核中':(item.isDoBusiness===1?'已审核':'状态未知')}}</p>
                  </div>
                </div>
...
//      console.log('deactivated')
    ...
    methods: {
      changeTab (item) {
        ...
        console.log(this.status2.pullupStatus)
      },
      getState (state) {
        if (state === 1) {
          return '已申请'
      ...
      loadMore1 () {
        if (this.status1.pullupStatus === 'disable') return false
      ...
      loadMore2 () {
        console.log(this.status2.pullupStatus)
        if (this.status2.pullupStatus === 'disable') return false
        this.$vux.loading.show({
          text: 'Loading'
        })
        setTimeout(() => {
